...
public class UploadControllerTest {
    ...
    // 1. We mock the S3Service. This prevents any real connection to AWS.
    @MockitoBean
    private S3Service s3Service;

    @Test
    public void uploadPng_shouldSucceed() throws Exception {
        // Mock behavior: When upload is called, return a fake URL immediately
        when(s3Service.uploadFile(any(MultipartFile.class)))
                .thenReturn("https://s3.amazonaws.com/bucket/test.png");
        ...
        mockMvc.perform(multipart("/upload")
                ...
                .andExpect(jsonPath("$.url").value(containsString("https://")));
    }

    @Test
    public void uploadTooLarge_shouldReject() throws Exception {
        // Create a dummy byte array slightly larger than 12MB
        byte[] big = new byte[12 * 1024 * 1024 + 1024];
        ...
        mockMvc.perform(multipart("/upload")
                ...
                .andExpect(jsonPath("$.error").value("File too large. Max size is 12MB."));
    }
    ...
    public void uploadDocx_shouldSucceed() throws Exception {
        when(s3Service.uploadFile(any(MultipartFile.class)))
                .thenReturn("https://s3.amazonaws.com/bucket/doc.docx");
    ...
    public void uploadOleDoc_shouldSucceed() throws Exception {
        when(s3Service.uploadFile(any(MultipartFile.class)))
                .thenReturn("https://s3.amazonaws.com/bucket/old.doc");
